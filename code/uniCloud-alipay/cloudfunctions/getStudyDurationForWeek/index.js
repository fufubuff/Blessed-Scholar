...
exports.main = async (event, context) => {
  // 获取本周的开始日期和结束日期
  const { weekStart, weekEnd } = event;  // weekStart 和 weekEnd 格式应该是 "YYYY-MM-DD"
  console.log('收到的 weekStart 参数:', weekStart);
  console.log('收到的 weekEnd 参数:', weekEnd);

  try {
    // 查询数据库中的 study_users 集合，筛选出 startDate 在 weekStart 到 weekEnd 之间的数据
    const studyUsers = await db.collection('study_users')
      .where({
        startDate: db.command.gte(weekStart),  // startDate >= weekStart
        startDate: db.command.lte(weekEnd)     // startDate <= weekEnd
      })
      .get();
    ...
    console.log('返回的学习时长数据:', result);  // 输出最终的学习时长数据
